...
class TCNUnitAttention(nn.Module, ABC):
    def __init__(self,
        ...
        if (self.in_channels != self.out_channels) or (stride != 1):
            self.down = Unit2D(self.in_channels,
                               self.out_channels,
                               kernel_size=1,
                               stride=stride)
        else:
        ...
        if not self.only_temporal_att:
            self.tcn_conv = Unit2D(in_channels, out_channels - self.dv, dropout=dropout,
                                   ...
                                   stride=self.stride)
        base_dim = 2 * self.dk + self.dv
        factor_dim = self.Nh // self.num if self.more_channels else 1
        self.qkv_conv = nn.Conv2d(self.in_channels, base_dim * factor_dim,
